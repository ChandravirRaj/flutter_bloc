...
class _ScreenCaptureExampleState extends State<ScreenCaptureExample> {
  ...
  Future<void> _createPdfAndShareSinglePage(ui.Image capturedImage) async {
    if (await Permission.storage.request().isGranted) {
      try {
        ...
        final int segmentHeight = (imageHeight / 3).ceil();
        ...
        for (int i = 0; i < 3; i++) {
          ...
          final segment = await _cropImageSegment(
              fullImageBytes, imageWidth, segmentHeight, i * segmentHeight);
        ...
        final String formattedTime = now
            .toIso8601String()
            .replaceAll(':', '-'); // You can customize the format
        final String filePath =
            "${downloadsDirectory.path}/health_report_$formattedTime.pdf";
  ...
  Future<Uint8List> _cropImageSegment(
    ...
    final Rect cropRect = Rect.fromLTWH(
        0, offsetY.toDouble(), imageWidth.toDouble(), segmentHeight.toDouble());
    ...
    final ui.Image croppedImage =
        await recorder.endRecording().toImage(imageWidth, segmentHeight);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Capture Screen Example"),
        actions: [
          IconButton(
        ...
        child: RepaintBoundary(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              // Ensures Column doesn't expand infinitely
              children: [
                // ElevatedButton(
                //   onPressed: () async {
                //     ui.Image image = await _captureImage();
